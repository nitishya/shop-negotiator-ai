...
function DeveloperCard({ dev }: { dev: typeof DEVELOPERS[0] }) {
  ...
  return (
    <div 
      className="flex flex-col gap-4 p-5 rounded-2xl border border-zinc-800 bg-zinc-900/40 hover:bg-zinc-900/80 transition-all cursor-pointer group shadow-md"
      onClick={() => setIsExpanded(!isExpanded)}
    >
      {/* Dev header */}
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-extrabold text-lg group-hover:scale-105 transition-transform">
          {dev.name.charAt(0)}
        </div>
        <div>
      ...
      <div className={`flex flex-col gap-2 overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[500px] mt-2 opacity-100' : 'max-h-0 opacity-0 m-0'}`}>
        ...
        {dev.linkedin && dev.linkedin !== 'https://www.linkedin.com/in/' && (
          <a
            ...
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-zinc-800 bg-zinc-950 hover:bg-indigo-500/10 hover:border-indigo-500/30 text-zinc-300 hover:text-white transition-all active:scale-[0.98]"
          >
            <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-indigo-500/10 text-indigo-400 flex-shrink-0">
        ...
        {dev.github && dev.github !== 'https://github.com/' && (
          <a
            ...
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-zinc-800 bg-zinc-950 hover:bg-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white transition-all active:scale-[0.98]"
          >
            <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-zinc-800 text-zinc-400 flex-shrink-0">
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {!isExpanded && (
        <div className="text-center w-full pt-1 opacity-50 group-hover:opacity-100 transition-opacity">
          <span className="text-[10px] uppercase font-bold text-zinc-500">Click to view details</span>
        </div>
      )}
    </div>
  );
